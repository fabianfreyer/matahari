 ...
 *
 * \file fat.c
 * \brief Petit FatFs - FAT file system module
 * \version R0.03
 * \author (C)ChaN, 2014
 *
 *
...
#include <stage2/fat.h>   /* Petit FatFs configurations and declarations */
#include <stage2/diskio.h>    /* Declarations of low level disk I/O functions */



/**
* \addtogroup pff_private Module Private Definitions
* @{
*/

#if _PFATFS != 4004 /*< Revision ID */
#error Wrong include file (fat.h).
#endif
...
/**
* \addtogroup pff_locale DBCS code ranges and SBCS extend char conversion table
* @{
*/
...
/**
* \defgroup pff_charcode Character code support macros
* @{
*/
...
/**
 * \defgroup pff_offset FatFS Offsets
 *
...
#if _FS_FAT16
  case FS_FAT16 :
    if (disk_readp(buf, fs->fatbase + clst / 256, ((uint32_t)clst % 256) * 2, 2)) break;
    return LD_uint16_t(buf);
#endif
#if _FS_FAT32
  case FS_FAT32 :
    if (disk_readp(buf, fs->fatbase + clst / 128, ((uint32_t)clst % 128) * 4, 4)) break;
    return LD_uint32_t(buf) & 0x0FFFFFFF;
#endif
  }
...
* \retval 0 Failed - invalid cluster#
...
/**
* \brief Get cluster field from directory entry
* \param  dir   Pointer to the directory entry
*/
static CLUST get_clust(uint8_t* dir) {
  FATFS *fs = FatFs;
  CLUST clst = 0;


  if (_FS_32ONLY || (_FS_FAT32 && fs->fs_type == FS_FAT32)) {
    clst = LD_uint16_t(dir+DIR_FstClusHI);
    clst <<= 16;
  }
  clst |= LD_uint16_t(dir+DIR_FstClusLO);
...
* \retval FR_NO_FILE  End of table
*/
...
/**
* \brief Directory handling - Find an object in the director
...
* \param  dir 32-byte working buffer
*/
static FRESULT dir_read(DIR *dj, uint8_t *dir) {
  FRESULT res;
  uint8_t a, c;
...
* \param  dj  Pointer to the directory object
* \param  dir 32-byte working buffer
* \param  fno ointer to store the file information
*/
static void get_fileinfo(DIR *dj, uint8_t *dir, FILINFO *fno) {
  uint8_t i, c;
  char *p;
  ...
  if (dj->sect) {
    ...
    fno->fsize = LD_uint32_t(dir+DIR_FileSize); /* Size */
    fno->fdate = LD_uint16_t(dir+DIR_WrtDate);    /* Date */
    fno->ftime = LD_uint16_t(dir+DIR_WrtTime);    /* Time */
  }
  *p = 0;
}
#endif /* _USE_DIR */
...
* \returns  Error code
*/
static FRESULT follow_path(DIR *dj, uint8_t *dir, const char *path) {
FRESULT res;
...
* \retval 0 FAT boot record
* \retval 1 Valid boot record but not FAT
* \retval 2 Not a boot record
* \retval 3 Error
*/
static uint8_t check_fs(uint8_t *buf, uint32_t sect) {
  if (disk_readp(buf, sect, 510, 2))    /* Read the boot record */
    return 3;
  if (LD_uint16_t(buf) != 0xAA55)       /* Check record signature */
    return 2;

  if (!_FS_32ONLY && !disk_readp(buf, sect, BS_FilSysType, 2) && LD_uint16_t(buf) == 0x4146)  /* Check FAT12/16 */
    return 0;
  if (_FS_FAT32 && !disk_readp(buf, sect, BS_FilSysType32, 2) && LD_uint16_t(buf) == 0x4146)  /* Check FAT32 */
    return 0;
  return 1;
}
...
FRESULT pf_mount (FATFS *fs) {
  ...
  if (fmt == 1) {           /* Not an FAT boot record, it may be FDISK format */
    ...
    if (disk_readp(buf, bsect, MBR_Table, 16)) {  /* 1st partition entry */
      ...
      if (buf[4]) {         /* Is the partition existing? */
        bsect = LD_uint32_t(&buf[8]); /* Partition offset in LBA */
  ...
  fsize = LD_uint16_t(buf+BPB_FATSz16-13);        /* Number of sectors per FAT */
  if (!fsize) fsize = LD_uint32_t(buf+BPB_FATSz32-13);

  fsize *= buf[BPB_NumFATs-13];           /* Number of sectors in FAT area */
  fs->fatbase = bsect + LD_uint16_t(buf+BPB_RsvdSecCnt-13); /* FAT start sector (lba) */
  fs->csize = buf[BPB_SecPerClus-13];         /* Number of sectors per cluster */
  fs->n_rootdir = LD_uint16_t(buf+BPB_RootEntCnt-13);   /* Nmuber of root directory entries */
  tsect = LD_uint16_t(buf+BPB_TotSec16-13);       /* Number of sectors on the file system */
  if (!tsect) tsect = LD_uint32_t(buf+BPB_TotSec32-13);
  mclst = (tsect            /* Last cluster# + 1 */
    - LD_uint16_t(buf+BPB_RsvdSecCnt-13) - fsize - fs->n_rootdir / 16
    ) / fs->csize + 2;
  ...
  if (_FS_32ONLY || (_FS_FAT32 && fmt == FS_FAT32))
    fs->dirbase = LD_uint32_t(buf+(BPB_RootClus-13)); /* Root directory start cluster */
  else
...
FRESULT pf_read(void* buff, uint32_t btr, uint32_t* br) {
  ...
  *br = 0;
  ...
  while (btr) {                 /* Repeat until all data transferred */
    ...
    rcnt = 512 - (uint32_t)fs->fptr % 512;      /* Get partial sector data from sector buffer */
    if (rcnt > btr) rcnt = btr;
    dr = disk_readp(!buff ? 0 : rbuff, fs->dsect, (uint32_t)fs->fptr % 512, rcnt);
...
FRESULT pf_write(const void* buff, uint32_t btw, uint32_t* bw) {
  ...
  *bw = 0;
  ...
  remain = fs->fsize - fs->fptr;
  if (btw > remain) btw = (uint32_t)remain;     /* Truncate btw by remaining bytes */

  while (btw) {                 /* Repeat until all data transferred */
    if ((uint32_t)fs->fptr % 512 == 0) {      /* On the sector boundary? */
      cs = (uint8_t)(fs->fptr / 512 & (fs->csize - 1)); /* Sector offset in the cluster */
      if (!cs) {                /* On the cluster boundary? */
        if (fs->fptr == 0)          /* On the top of the file? */
          clst = fs->org_clust;
    ...
    wcnt = 512 - (uint32_t)fs->fptr % 512;      /* Number of bytes to write to the sector */
    if (wcnt > btw) wcnt = btw;
    if (disk_writep(p, wcnt)) ABORT(FR_DISK_ERR); /* Send data to the sector */
    fs->fptr += wcnt; p += wcnt;        /* Update pointers and counters */
    btw -= wcnt; *bw += wcnt;
    if ((uint32_t)fs->fptr % 512 == 0) {
